...
class lihatTweet(Resource):
    def post(self):
        ...
        for dt in Tweets:
            for d in Tweets[dt]:
                if dt == email and d == tweet:
                    return "Kapanggih"

        return "Teu kapanggih"

class ubahTweet(Resource):
    def post(self):
        email = request.json["email"]
        tweet = request.json["tweet"]
        ...
        for data in Tweets:
            for idx,tw in enumerate(Tweets[data]):
                if data == email and tw == tweet:
                    Tweets[data][idx] = tweetbaru
                    tambahTweet()
                    return Tweets, 201
...
api.add_resource(lihatTweet,'/lihatTweet')
api.add_resource(getUser,'/getUser')
api.add_resource(ubahTweet,'/ubahTweet')